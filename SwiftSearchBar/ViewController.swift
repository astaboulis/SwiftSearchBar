...
//  SwiftSearchBar
//
//  Created by Angelos Staboulis on 27/7/20.
...
class ViewController: UITableViewController {
    ...
    @objc func textFieldEditing(sender:UITextField){
        self.filtered = self.names.filter({ (event) -> Bool in
            return event.contains(String(search.text!))
        })
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.filtered = self.names
        self.search.addTarget(self, action: #selector(textFieldEditing(sender:)), for: .allEditingEvents)
